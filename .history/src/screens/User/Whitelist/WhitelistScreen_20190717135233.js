...
import {
    ...
    BUTTONS_ACTION_REMOVE_WHITELIST, CANCEL_INDEX_ACTION__REMOVE_WHITELIST,
...
import ListFollowingItem from 'screens/home/ListFollowingItem';

export default class WhitelistScreen extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data: [],
            ...
            if (!this.state.loading) {
                if (this.state.data && this.state.data.length > 0) {
                    return <FlatList
                        ...
                        style={{ paddingBottom: 40, height: height, backgroundColor: R.colors.white100 }}
                    ...
                    return (
                        <View style={styles.viewNotData}>
                            <Text style={styles.textNotData}>No Whitelist</Text>
                        </View>
                    )
                }
            }
    ...
    onAction = () => {
        ActionSheetIOS.showActionSheetWithOptions({
            options: BUTTONS_ACTION_REMOVE_WHITELIST,
            cancelButtonIndex: CANCEL_INDEX_ACTION__REMOVE_WHITELIST,
        },
            (buttonIndex) => {
                switch (buttonIndex) {
                    case 0:
    ...
    onRemoveWhitelist =  () => {
        ...
        let data = this.state.data.filter(item => item.status_check).map(value => value.id);
        let objectUser = {
            id: data.join(','),
        }
        ...
        apis.post(apis.PATH.REMOVE_WHITELIST, objectUser).then(response => {
            // Refresh data
            if (response.code == Status.SUSSESS) {
                const newData = this.state.data.filter(item => !data.includes(item.id));
