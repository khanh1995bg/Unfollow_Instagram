...
import {
    ...
    BUTTONS_ACTION_MEDIA, CANCEL_INDEX_ACTION_MEDIA,
...
export default class MediaScreen extends Component {
    ...
    getListMedia = async () => {
        ...
        apis.fetch(apis.PATH.MEDIA, objectUser).then(response => {
    ...
    _renderItem = ({ item, index }) => {
        return <ListMediaItem item={item} index={index} onChooseItem={this.onChooseItem} />
    }
        ...
        if (!this.state.loadingPage) {
            if (!this.state.loading) {
                if (this.state.data && this.state.data.length > 0) {
                    return <FlatList
                    style={{ paddingTop: 10, paddingBottom: 40, backgroundColor: R.colors.white100, height: height }}
                    ...
                    return (
                        <View style={styles.viewNotData}>
                            <Text style={styles.textNotData}>No Post</Text>
                        </View>
                    )
                }
            }
    ...
    onAction = () => {
        ActionSheetIOS.showActionSheetWithOptions({
            options: BUTTONS_ACTION_MEDIA,
            cancelButtonIndex: CANCEL_INDEX_ACTION_MEDIA,
        },
            (buttonIndex) => {
                switch (buttonIndex) {
                    case 0:
    ...
    onDelete = async () => {
        ...
        Alert.alert(
            'Confirm Deletion',
            `Are you sure you want to delete ${CountProfile.length} profiles`,
            [
                { text: 'No' },
                { text: 'Do it', onPress: () => this.onDeleteMedia(objectUser) },
            ],
            { cancelable: false }
        )
    }
    onDeleteMedia = (objectUser) => {
        apis.post(apis.PATH.DEL_MEDIA, objectUser).then(response => {
            // Refresh data
            if (response.code == Status.SUSSESS) {
                ...
                this.setState({
                    data: newData,
                    isVisible: true,
                    textSelect: `Deleted ${idSelecteds.length} success!`
                }, () => {
                    setTimeout(() => {
                        this.setState({ isVisible: false })
                    }, 1000)
