...
class HomeScreen extends Component {
  ...
  getDataFollow = async () => {
    ...
    apis
      .fetch(apis.PATH.FOLLOWING, objectUser)
      .then(response => {
  ...
  _renderModal() {
    return <ModalCheckSuccess isVisible={this.state.isVisible} text={this.state.textSelect} />;
  }
  ...
  render() {
    ...
    return (
      <Container>
        <Header back text="Following" countSelect={countSelect} countTotal={data && data.length > 0 ? data.length : 0} showCount />
  ...
  onChooseAvatar = index => () => {
    const listItem = this.state.data.reduce(
      (prevValue, value, idx) => {
        if (index === idx) {
  ...
  onInvertSelect = () => {
    ...
    const listAllItem = this.state.data.reduce(
      (prevValue, value, idx) => {
        value.status_check = !value.status_check;
        prevValue.data.push(value);
        if (value.status_check) {
  ...
  onSelectNonFollowers = () => {
    if (database.unlockedAllFeatures) {
      Alert.alert(
        ...
        [{ text: 'Cancel' }, { text: 'Ok', onPress: () => this.onNonFollower() }],
        { cancelable: false }
      );
    } else {
  ...
  onSelectGhostUser = () => {
    if (database.unlockedAllFeatures) {
      Alert.alert(
        'Ghost users!',
        `Ghosts are users that have never liked or commented on your latest ${
          this.state.ghostFilter
        } posts. Filtering ghost followers may take a few minutes if you have a lot of comments.`,
        [{ text: 'Cancel' }, { text: 'Ok', onPress: () => this.onGhostUser(this.state.ghostFilter) }],
        { cancelable: false }
      );
    } else {
      NavigationService.navigate('InAppPurchaseScreen');
  ...
  onSelectInactiveUsers = () => {
    if (database.unlockedAllFeatures) {
      Alert.alert(
        ...
        [{ text: 'Cancel' }, { text: 'Ok', onPress: () => this.onInactive(this.state.inactiveDay) }],
        { cancelable: false }
      );
    } else {
      NavigationService.navigate('InAppPurchaseScreen');
  ...
  onUnFollow = async () => {
    ...
    Alert.alert(
      ...
      [{ text: 'No' }, { text: 'Do it', onPress: () => this.onDoneUnFollow(objectUser, data) }],
      { cancelable: false }
    );
  };
